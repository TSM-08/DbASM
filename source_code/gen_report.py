...
class MigrationReport:
    ...
    _NON_BLOCKING_SEVERITIES = {"LOW", "MEDIUM"}
    ...
    def __init__(self, config: dict, report_code: str, assessment: SchemaAssessment, 
                 connection: dict | None = None, timestamp: datetime | None = None):
        self._config = config.get("assessment", {})
        self._report_code = report_code
        self._report_name = utils.get_config_item(self._config, report_code, 'name', default='Migration Assessment Report')
        self._data_query = utils.get_config_item(self._config, report_code, 'data_query', default={})
        self._report_items = utils.get_config_item(self._config, report_code, 'items', default={})
        self._show_statistics = utils.get_config_item(self._config, report_code, 'show_statistics', default=True)
        self._section_length = utils.get_config_item(self._config, report_code, 'section_length', default=AppBase.SECTION_LENGTH)
        self._created_at = timestamp or datetime.now()
        self._report_file = f"post_assessment_{self._created_at:%Y%m%d%H%M%S}.rpt"
        self._assessment = assessment
        self._connection = connection.get('database') if connection else None

        # Statistics tracking
        self._total_rules = 0
        self._total_passed = 0
        self._total_failed = 0
        self._high_severity_failed = 0
        self._section_stats = {}
        self._status = "PASS"
    ...
    def _normalize_severity(self, severity: str) -> str:
        """Normalize severity values for internal logic."""
        ...
        return normalized_map.get(normalized, "UNKNOWN")

    def _normalize_severity_for_display(self, severity: str) -> str:
        """Normalize severity values to match config.yaml display casing."""
        if not isinstance(severity, str):
            return self.SEVERITY_UNKNOWN

        normalized = severity.strip().upper()
        return self._SEVERITY_DISPLAY_MAP.get(normalized, self.SEVERITY_UNKNOWN)

    def _is_non_blocking_failure(self, severity: str) -> bool:
        return severity in self._NON_BLOCKING_SEVERITIES
    ...
    def generate_report(self) -> str:
        ...
        migration_descr = ''
        if self._connection:
            if (self._connection.get('src', {}).get('type')):
                migration_descr += f"Source: {self._connection['src']['type']}"
            if (self._connection.get('trg', {}).get('type')):
                migration_descr += f" -> Target: {self._connection['trg']['type']}"
        else:
        ...
        report_lines.append("-"*self._section_length)
        ...
        for section, details in self._report_items.items():
            ...
            if status == "Fail":
                self._status = "FAIL"
                mark = ' <'
            report_lines.append(f"\n{section}. {title.ljust(self._section_length-8, '.')} {status.upper()}{mark}")
    ...
    def _get_summary_lines(self) -> list:
        ...
        lines.append("\n" + "="*self._section_length)
        lines.append("*** ASSESSMENT SUMMARY ***".center(self._section_length))
        lines.append("-"*self._section_length)
        ...
        lines.append(f"[H] High Severity Failures: {self._high_severity_failed}")
        lines.append(f"[R] Success Rate: {overall_success_rate:.1f}%")
        lines.append(f"[S] Overall Status: {overall_status}")

        if self._status == "FAIL":
            self._description = f"NOT READY - {self._high_severity_failed} high severity issues need attention"
    ...
    def _get_statistics_lines(self) -> list:
        ...
        lines.append("\n" + "="*self._section_length)
        lines.append("*** MIGRATION ASSESSMENT STATISTICS ***".center(self._section_length))
        lines.append("-"*self._section_length)
    ...
    def check_section_status(self, rules: list[dict]):
        ...
        """
        all_passed = True
        titles = utils.get_config_item(self._config, self._data_query, default={})
        details = []

        for rule in rules:
            for rule_id, severity in rule.items():
                severity_display = self._normalize_severity_for_display(severity)
                ...
                if rule_success:
                    ...
                    if not self._is_non_blocking_failure(severity):
                        self._high_severity_failed += 1
                
                # Determine if this rule affects section status
                if not rule_success and self._is_non_blocking_failure(severity):
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            'overall': {
                ...
                'high_severity_failed': self._high_severity_failed,
