...
class SchemaAssessment:
    ...
    def __init__(self, db_type: str, config: dict, direction: str):
        ...
        self.hash_qry_id = None
        self._hash_files = {}
        self._direction = direction

        self._data_query = {}
        self._data_query["metadata_query"] = config.get('metadata_query', {})
        self._data_query["validate_query"] = config.get('validate_query', {})
        self._data_query["finalchk_query"] = config.get('finalchk_query', {})
        self._data_query["precheck_query"] = config.get('precheck_query', {})
        self._data_query["incrment_query"] = config.get('incrment_query', {})
        self._data_query["rollback_query"] = config.get('rollback_query', {})
    ...
    def get_precheck_list(self):
        return self.get_data_list('precheck_query')

    def get_incrment_list(self):
        return self.get_data_list('incrment_query')

    def get_rollback_list(self):
        return self.get_data_list('rollback_query')
    ...
    def add_precheck(self, qry_id, data):
        self.add_fetched_data('precheck_query', qry_id, data)

    def add_incrment(self, qry_id, data):
        self.add_fetched_data('incrment_query', qry_id, data)
    
    def add_rollback(self, qry_id, data):
        self.add_fetched_data('rollback_query', qry_id, data)
    ...
    def get_precheck(self, qry_id: str) -> dict:
        return self.get_data_by_id('precheck_query', qry_id)

    def get_incrment(self, qry_id: str) -> dict:
        return self.get_data_by_id('incrment_query', qry_id)
    
    def get_rollback(self, qry_id: str) -> dict:
        return self.get_data_by_id('rollback_query', qry_id)
    ...
    def iterate_metadata_query(self):
        ...
        metadata_query = self._data_query["metadata_query"]
    ...
    def iterate_validate_query(self):
        ...
        validate_query = self._data_query["validate_query"]
        for qry_id, description in validate_query.items():
            sql = utils.extract_code_block(file_sql, qry_id)
            yield qry_id, description, sql
    
    def iterate_finalcheck_query(self):
        file_sql = AppBase.get_sql_path(self.FINALCHK_FILE)
        if not utils.file_exists(file_sql):
            raise FileNotFoundError(f"Final check SQL file not found: {file_sql}")

        finalchk_query=self._data_query["finalchk_query"]
        for qry_id, description in finalchk_query.items():
            sql = utils.extract_code_block(file_sql, qry_id)
            yield qry_id, description, sql

    def iterate_precheck_query(self):
        file_sql = AppBase.get_sql_path(self.INCRMENT_FILE)
        if not utils.file_exists(file_sql):
            raise FileNotFoundError(f"Precheck SQL file not found: {file_sql}")

        precheck_query = self._data_query["precheck_query"]
        for qry_id, description in precheck_query.items():
            sql = utils.extract_code_block(file_sql, qry_id)
            yield qry_id, description, sql

    def iterate_incrment_query(self):
        file_sql = AppBase.get_sql_path(self.INCRMENT_FILE)
        if not utils.file_exists(file_sql):
            raise FileNotFoundError(f"Discrepancy SQL file not found: {file_sql}")

        incrment_query = self._data_query["incrment_query"]
        for qry_id, description in incrment_query.items():
            sql = utils.extract_code_block(file_sql, qry_id)
            yield qry_id, description, sql

    def iterate_rollback_query(self):
        file_sql = AppBase.get_sql_path(self.FINALCHK_FILE)
        if not utils.file_exists(file_sql):
            raise FileNotFoundError(f"Rollback SQL file not found: {file_sql}")

        rollback_query = self._data_query["rollback_query"] 
        for qry_id, description in rollback_query.items():
            sql = utils.extract_code_block(file_sql, qry_id)
            yield qry_id, description, sql
...
class SchemaProcessor:
    ...
    def fetch_metadata_by_id(self, query: list):
        ...
        cols, rows = self.db.prepare_data(sql)
    ...
    @classmethod
    def run_precheck(cls, assessment: SchemaAssessment, db: DbConnector):
        for query in assessment.iterate_precheck_query():
            qry_id, description, sql = query
            ...
            if not sql:
                raise ValueError(SQL_CODE_NOT_EXISTS)
            cols, rows = db.prepare_data(sql)
            print(f"Found {len(rows) if rows else 0} issue(s)")

            assessment.add_precheck(qry_id, (cols, rows))
    
    @classmethod
    def run_discrepancy(cls, assessment: SchemaAssessment, db: DbConnector):
        for query in assessment.iterate_incrment_query():
            qry_id, description, sql = query
            if qry_id.startswith('DIS'):
                print(f"\nDetect changes: [{qry_id}] {description}...")
            else:
                print(f"\nTransformation: [{qry_id}] {description}...")

            if not sql:
                raise ValueError(SQL_CODE_NOT_EXISTS)
            cols, rows = db.prepare_data(sql)
            print(f"Found {len(rows) if rows else 0} row(s)")

            assessment.add_incrment(qry_id, (cols, rows))

    @classmethod
    def run_rollback_check(cls, assessment: SchemaAssessment, db: DbConnector):
        for query in assessment.iterate_rollback_query():
            qry_id, description, sql = query
            ...
            assessment.add_rollback(qry_id, (cols, rows))
